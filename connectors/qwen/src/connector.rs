...
use crate::types::{
    chat_request_from_value, count_image_url_blocks, embeddings_request_from_value,
    validate_qwen_model_id,
};

pub const CONNECTOR_ID: &str = "fcp.qwen";
...
impl QwenConfig {
    fn from_params(params: &Value) -> FcpResult<Self> {
        ...
        let auth = match (inline_auth, credential_id) {
            (Some(key), None) => QwenAuth::ApiKey(key),
            (None, Some(id)) => QwenAuth::CredentialId(id),
            (Some(_), Some(_)) => {
        ...
        let base_url = normalize_qwen_base_url(params.get("base_url").and_then(Value::as_str))
            .map_err(invalid_config)?;
        let default_model = optional_model(params, "default_model", DEFAULT_MODEL)?;
        let default_vision_model =
            optional_model(params, "default_vision_model", DEFAULT_VISION_MODEL)?;
    ...
    fn build_client(&self) -> QwenClient {
        QwenClient::new(
            QwenProvider::new(self.base_url.clone(), self.auth.clone()),
...
impl QwenConnector {
    ...
    pub async fn handle_configure(&mut self, params: Value) -> FcpResult<Value> {
        let config = QwenConfig::from_params(&params)?;
        let client = config.build_client();
        let auth_mode = config.auth.redacted_label();
        let base_url = config.base_url.clone();
        let default_model = config.default_model.clone();
        let default_vision_model = config.default_vision_model.clone();
        let default_embedding_model = config.default_embedding_model.clone();
        self.client = Some(Arc::new(client));
        self.config = Some(config);
        self.base.set_configured(true);
        info!(auth = %auth_mode, base_url = %base_url, "Qwen connector configured");
        Ok(json!({
    ...
    pub async fn handle_health(&self) -> FcpResult<Value> {
        ...
        Ok(json!({
            ...
            "default_vision_model": self.config.as_ref().map(|config| config.default_vision_model.clone()),
        ...
        Ok(json!({
            ...
            "checks": [
                ...
                },
                {
                    ...
                    "passed": self.config.as_ref().is_none_or(|config| config.base_url == DEFAULT_BASE_URL || config.base_url == BEIJING_BASE_URL || config.base_url.contains("127.0.0.1") || config.base_url.contains("localhost")),
                    "critical": true,
                    "message": "base_url is constrained to DashScope OpenAI-compatible mode, with loopback allowed for tests"
                },
                {
                    "name": "qwen_vl_shape",
                    "passed": true,
                    "critical": false,
                    "message": "Qwen-VL multimodal input uses standard OpenAI image_url content blocks"
                },
                {
                    "name": "dashscope_native_endpoint_denied",
                    "passed": true,
                    "critical": true,
                    "message": "DashScope-native /api/v1/services endpoints are intentionally out of scope"
                },
                {
                    "name": "handshake",
    ...
    async fn invoke_operation(&self, operation: &str, input: Value) -> FcpResult<Value> {
        ...
        let client = self.client.as_ref().ok_or_else(|| FcpError::Internal {
            message: "Qwen client not initialized".into(),
        })?;
        ...
        match operation {
            OP_CHAT => {
                let image_url_count = input
                    .get("messages")
                    .cloned()
                    .and_then(|messages| {
                        serde_json::from_value::<Vec<fcp_openai_compat::ChatMessage>>(messages).ok()
                    })
                    .map_or(0, |messages| count_image_url_blocks(&messages));
                ...
                client
                    ...
                    .await
                    .map(|response| chat_response_to_value(response, image_url_count))
            ...
            OP_HEALTH => {
                ...
                Ok(json!({
                    "status": "ok",
                    "provider": "qwen",
                    "endpoint_class": endpoint_class(&config.base_url),
...
fcp_core::impl_fcp_sealed!(QwenConnector);

#[fcp_core::async_trait]
impl FcpConnector for QwenConnector {
    ...
    async fn health(&self) -> HealthSnapshot {
        if self.config.is_some() && self.session_id.is_some() {
            ...
            HealthSnapshot::degraded("qwen_handshake_pending")
        } else {
            HealthSnapshot::error("qwen_not_configured")
        }
    }
    ...
    async fn self_check(&self) -> FcpResult<SelfCheckReport> {
        if self.config.is_none() {
            return Ok(SelfCheckReport::degraded(
                "not_configured",
                "Qwen connector is not configured",
            ));
        }
        if self
            .config
    ...
    async fn simulate(&self, req: SimulateRequest) -> FcpResult<SimulateResponse> {
        if is_supported_operation(req.operation.as_str()) {
            ...
            Ok(SimulateResponse::denied(
                req.id,
                "operation is not supported by Qwen",
                "FCP-3010",
            ))
        }
    }
    ...
    OPERATIONS
        .get_or_init(|| {
            typed_operations_info()
                .expect("embedded Qwen manifest should validate for introspection")
        })
        .clone()
}
...
fn resource_uris_for_operation(operation: &str, input: &Value) -> Vec<String> {
    match operation {
        ...
        OP_EMBEDDINGS => {
            ...
            vec![format!("qwen:embedding-model:{model}")]
        }
        OP_MODELS | OP_HEALTH => vec!["qwen:models".into()],
        _ => Vec::new(),
    }
}
...
fn optional_model(params: &Value, field: &str, default: &str) -> FcpResult<String> {
    ...
    validate_qwen_model_id(field, raw)
}
...
fn endpoint_class(base_url: &str) -> &'static str {
