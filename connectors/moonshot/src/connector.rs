...
use crate::client::{
    DEFAULT_BASE_URL, DEFAULT_MODEL, MoonshotAuth, MoonshotClient, MoonshotProvider,
    normalize_moonshot_base_url, validate_auth_material,
};
use crate::error::openai_error_to_fcp;
use crate::types::{
    DEFAULT_CONTEXT_WINDOW_TOKENS, chat_request_from_value, context_window_class,
...
impl MoonshotConfig {
    fn from_params(params: &Value) -> FcpResult<Self> {
        ...
        let auth = match (inline_auth, credential_id) {
            (Some(key), None) => MoonshotAuth::ApiKey(key),
            (None, Some(id)) => MoonshotAuth::CredentialId(id),
            (Some(_), Some(_)) => {
        ...
        let base_url = normalize_moonshot_base_url(params.get("base_url").and_then(Value::as_str))
            .map_err(invalid_config)?;
        let default_model = params
            ...
            .to_string();
        let default_context_window_tokens = optional_positive_u32(
            params,
            "default_context_window_tokens",
            context_window_for_model(&default_model).unwrap_or(DEFAULT_CONTEXT_WINDOW_TOKENS),
        )?;
    ...
    fn build_client(&self) -> MoonshotClient {
        MoonshotClient::new(
            MoonshotProvider::new(self.base_url.clone(), self.auth.clone()),
...
pub struct MoonshotConnector {
    base: Arc<BaseConnector>,
    config: Option<MoonshotConfig>,
    client: Option<Arc<MoonshotClient>>,
...
impl MoonshotConnector {
    ...
    pub async fn handle_configure(&mut self, params: Value) -> FcpResult<Value> {
        ...
        let default_context_window_tokens = config.default_context_window_tokens;
        self.client = Some(Arc::new(client));
        self.config = Some(config);
        self.base.set_configured(true);
        info!(auth = %auth_mode, base_url = %base_url, "Moonshot connector configured");
        Ok(json!({
            ...
            "context_window_class": context_window_class(default_context_window_tokens),
        }))
    }
    ...
    pub async fn handle_health(&self) -> FcpResult<Value> {
        ...
        Ok(json!({
            ...
            "default_context_window_tokens": self.config.as_ref().map(|config| config.default_context_window_tokens),
        ...
        Ok(json!({
            ...
            "checks": [
                ...
                },
                {
                    ...
                    "critical": true,
                    "message": "base_url is constrained to api.moonshot.ai/v1 or api.moonshot.cn/v1, with loopback allowed for tests"
                },
                {
                    "name": "context_limit_policy",
                    "passed": true,
                    "critical": true,
                    "message": "estimated_input_tokens plus requested output is rejected when it exceeds the configured context window"
                },
                {
                    "name": "handshake",
    ...
    async fn invoke_operation(&self, operation: &str, input: Value) -> FcpResult<Value> {
        ...
        match operation {
            OP_CHAT => {
                ...
                let context_class = response_context_window_class(
                    &request.model,
                    config.default_context_window_tokens,
                );
                client
                    ...
                    .await
                    .map(|response| chat_response_to_value(&response, context_class))
            ...
            OP_CHAT_STREAM => {
                ...
                let stream = client
                    ...
                    .await
                    .map_err(|err| openai_error_to_fcp(&err))?;
                collect_stream_response(stream, context_class).await
            }
            OP_MODELS => {
                if input
                    .get("refresh")
            ...
            OP_HEALTH => {
                ...
                Ok(json!({
                    ...
                    "context_window_class": context_window_class(config.default_context_window_tokens),
                }))
            }
            OP_EMBEDDINGS => Err(FcpError::InvalidRequest {
                code: 1003,
                message: "moonshot.embeddings.create is not supported by the current first-party Kimi API; use another embeddings connector".into(),
            }),
    ...
    pub async fn handle_simulate(&self, params: Value) -> FcpResult<Value> {
        ...
        Ok(json!({
            ...
            "reason": if operation == OP_EMBEDDINGS {
                "Moonshot embeddings are intentionally not supported by this connector because current Kimi docs do not expose /v1/embeddings."
...
impl Default for MoonshotConnector {
    fn default() -> Self {
        Self::new()
    }
}

fcp_core::impl_fcp_sealed!(MoonshotConnector);

#[fcp_core::async_trait]
impl FcpConnector for MoonshotConnector {
    ...
    async fn health(&self) -> HealthSnapshot {
        if self.config.is_some() && self.session_id.is_some() {
            ...
            HealthSnapshot::degraded("moonshot_handshake_pending")
        } else {
            HealthSnapshot::error("moonshot_not_configured")
        }
    }
    ...
    async fn self_check(&self) -> FcpResult<SelfCheckReport> {
        if self.config.is_none() {
            return Ok(SelfCheckReport::degraded(
                "not_configured",
                "Moonshot connector is not configured",
            ));
        }
        if self
            .config
    ...
    async fn simulate(&self, req: SimulateRequest) -> FcpResult<SimulateResponse> {
        if matches!(
            ...
            Ok(SimulateResponse::denied(
                req.id,
                "operation is not supported by Moonshot",
                "FCP-3010",
            ))
        }
    }
    ...
    match operation {
        OP_CHAT | OP_CHAT_STREAM => {
            let model = input
                ...
                .and_then(Value::as_str)
                .unwrap_or(DEFAULT_MODEL);
            vec![format!("moonshot:model:{model}")]
        }
        OP_MODELS | OP_HEALTH | OP_EMBEDDINGS => vec!["moonshot:models".into()],
        _ => Vec::new(),
    }
}
...
fn optional_positive_u32(params: &Value, field: &str, default: u32) -> FcpResult<u32> {
    match optional_positive_u64(params, field, u64::from(default))? {
        value if value > u64::from(u32::MAX) => Err(FcpError::InvalidRequest {
            code: 1003,
            message: format!("{field} must fit in u32"),
        }),
        value => u32::try_from(value).map_err(|err| FcpError::Internal {
            message: format!("Failed to convert {field} to u32 after range validation: {err}"),
        }),
    }
}

const fn invalid_config(message: String) -> FcpError {
...
fn response_context_window_class(model: &str, fallback_tokens: u32) -> &'static str {
    context_window_class(context_window_for_model(model).unwrap_or(fallback_tokens))
}

fn chat_response_to_value(
    response: &fcp_openai_compat::ChatCompletionsResponse,
    context_window_class: &'static str,
) -> Value {
