...
| `rch exec -- cargo ...` fails with `RCH-E326` or `fatal: bad object HEAD` | Worker-side canonical clone synced git refs without matching object database | Treat as worker clone state; verify `/data/projects/flywheel_connectors` with `git cat-file -t HEAD`; refresh refs without rewriting the worktree |
