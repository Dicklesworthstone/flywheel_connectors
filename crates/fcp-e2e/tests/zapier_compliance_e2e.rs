//! E2E Zapier connector compliance tests.
//!
//! Exercises the Zapier connector through the shared E2E harness:
...
//! Run: `cargo test --package fcp-e2e --features zapier`
...
impl ZapierConnectorAdapter {
    fn new() -> Self {
        Self {
            connector: ZapierConnector::new(),
...
impl FcpConnector for ZapierConnectorAdapter {
    ...
    async fn handshake(&mut self, req: HandshakeRequest) -> fcp_core::FcpResult<HandshakeResponse> {
        ...
        let protocol_version = response
            ...
            .ok_or_else(|| FcpError::Internal {
                message: "zapier handshake response missing protocol_version".into(),
            })?;
        if protocol_version != "2.0" {
            return Err(FcpError::Internal {
                message: format!(
                    "zapier handshake protocol_version expected 2.0, got {protocol_version}"
                ),
            });
        }
        let _connector_id = response
            ...
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| FcpError::Internal {
                message: "zapier handshake response missing connector_id".into(),
            })?;
        let connector_caps: std::collections::BTreeSet<String> = response
            .get("capabilities")
            .and_then(serde_json::Value::as_array)
            .ok_or_else(|| FcpError::Internal {
                message: "zapier handshake response missing capabilities array".into(),
            })?
            .iter()
        ...
        Ok(HandshakeResponse {
            ...
            session_id,
            manifest_hash: "sha256:zapier-e2e".to_string(),
            nonce: req.nonce,
            event_caps: None,
            auth_caps: None,
        ...
        match self.connector.handle_health().await {
            Ok(payload) => {
                ...
                match status {
                    ...
                    other => HealthSnapshot::degraded(format!("zapier_status:{other}")),
                }
            }
    ...
    async fn self_check(&self) -> fcp_core::FcpResult<fcp_core::SelfCheckReport> {
        ...
        serde_json::from_value(value).map_err(|err| FcpError::Internal {
            message: format!("failed to deserialize Zapier self_check: {err}"),
        })
    }
    ...
    fn introspect(&self) -> Introspection {
        Introspection {
            operations: vec![OperationInfo {
                id: OperationId::from_static("zapier.zaps.list"),
                summary: "zapier.zaps.list".to_string(),
                description: None,
                input_schema: json!({"type": "object"}),
                output_schema: json!({"type": "object"}),
                capability: CapabilityId::from_static("zapier.zaps.read"),
    ...
    async fn invoke(&self, req: InvokeRequest) -> fcp_core::FcpResult<InvokeResponse> {
        let verifier = self.verifier.as_ref().ok_or(FcpError::Internal {
            message: "Zapier verifier not initialized; handshake required".into(),
        })?;
...
fn required_capability(operation: &str) -> fcp_core::FcpResult<CapabilityId> {
    let capability = match operation {
        "zapier.zaps.list" => "zapier.zaps.read",
        "zapier.zaps.execute" => "zapier.zaps.write",
        _ => {
...
fn zapier_manifest_with_hash() -> String {
    let raw = include_str!("../../../connectors/zapier/manifest.toml");
...
fn zapier_manifest_toml() -> toml::Value {
    toml::from_str(include_str!("../../../connectors/zapier/manifest.toml"))
        .expect("Zapier manifest TOML")
}

fn zapier_config(base_url: &str) -> serde_json::Value {
    json!({
        "api_key": "zapier-test-api-key",
        "base_url": base_url,
    })
}
...
fn invoke_request(
    ...
    InvokeRequest {
        ...
        id: RequestId::from("zapier-e2e"),
        connector_id: ConnectorId::from_static("zapier"),
...
async fn zapier_default_deny_compliance_suite_passes() {
    let mock = MockApiServer::start().await;

    let mut connector = ZapierConnectorAdapter::new();
...
#[fcp_async_core::runtime::test]
async fn zapier_allow_valid_token_connector_suite_passes() {
    let mock = MockApiServer::start().await;
...
#[test]
fn zapier_manifest_network_guard_allows_and_denies() {
    let manifest = zapier_manifest_toml();
    ...
    for operation_name in operations.keys() {
        ...
        assert_eq!(
            ...
            "operation {operation_name} should use exact Zapier API host allowlist"
        );

        assert!(host_allowed("nla.zapier.com", &host_allow));
        assert!(host_allowed("api.zapier.com", &host_allow));
        assert!(!host_allowed("zapier.com", &host_allow));
        assert!(!host_allowed("evil.nla.zapier.com", &host_allow));
